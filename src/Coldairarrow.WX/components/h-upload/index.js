...
import { isObject, SuperComponent, wxComponent } from 'tdesign-miniprogram/common/src/index';
import ActionSheet, { ActionSheetTheme } from 'tdesign-miniprogram/action-sheet/index';
import props from './props';
import config from 'tdesign-miniprogram/common/config';
import { isOverSize } from 'tdesign-miniprogram/common/utils';
import { getExtension } from "@utils/util.js"
const { prefix } = config;
...
let Upload = class Upload extends SuperComponent {
  constructor() {
      ...
      this.methods = {
          ...
          chooseMessageFile(mediaType) {
              const { max, config, sizeLimit, extensions } = this.properties;
  ...
  onProofTap(e) {
      ...
      wx.previewImage({
          urls: this.data.customFiles.filter((file) => file.type === "image").map((file) => file.url),
  ...
  onDocumentClick(e) {
    var _a;
    const { index } = e.currentTarget.dataset;
    const url = (_a = this.data.customFiles[index]) === null || _a === void 0 ? void 0 : _a.url;
    const name = (_a = this.data.customFiles[index]) === null || _a === void 0 ? void 0 : _a.name;
