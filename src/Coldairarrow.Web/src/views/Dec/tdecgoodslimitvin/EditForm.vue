  ...
  >
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label="VIN序号" prop="VinNo">
          <a-input v-model="entity.VinNo" autocomplete="off" />
        </a-form-model-item>
...
export default {
  ...
  methods: {
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.loading = true
        this.$http.post('/Dec/tdecgoodslimitvin/GetTheData', { id: id }).then(resJson => {
          this.loading = false
    ...
    handleSubmit() {
      this.$refs['form'].validate(valid => {
        ...
        this.loading = true
        this.$http.post('/Dec/tdecgoodslimitvin/SaveData', this.entity).then(resJson => {
          this.loading = false
