  ...
  >
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label="集装箱自重" prop="ContainerWt">
          <a-input v-model="entity.ContainerWt" autocomplete="off" />
        </a-form-model-item>
        <a-form-model-item label="拼箱标识" prop="LclFlag">
...
export default {
  ...
  methods: {
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.loading = true
        this.$http.post('/IAQ/itf_dcl_io_decl_cont_detail/GetTheData', { id: id }).then(resJson => {
          this.loading = false
    ...
    handleSubmit() {
      this.$refs['form'].validate(valid => {
        ...
        this.loading = true
        this.$http.post('/IAQ/itf_dcl_io_decl_cont_detail/SaveData', this.entity).then(resJson => {
          this.loading = false
